...
/**
 * SNN EDU Comment List element (inline edit / delete ready, with delete button for admins/editors only)
 */
class SNN_EDU_Element_Comment_List extends Element {
    public $category     = 'snn';
    public $name         = 'snn-edu-comment-list';
    public $icon         = 'ti-comments';
    public $css_selector = '.snn-commentlist';
    public $nestable     = false;
    public function get_label() { return esc_html__( 'SNN EDU Comment List', 'snn' ); }
...
<?php endif; ?>
        ...
        function snn_edu_get_user_initials( $display_name ) {
        ...
        function snn_edu_user_can_delete_comment() {
        ...
        if ( ! function_exists( 'snn_edu_comment_callback' ) ) {
            function snn_edu_comment_callback( $comment, $args, $depth ) {
...
<li <?php comment_class( 'snn-comment-item' ); ?> id="comment-<?php comment_ID(); ?>">
    <comment id="div-comment-<?php comment_ID(); ?>" class="snn-comment-body">
        ...
        wp_list_comments( [
            'style'       => 'ul',
            'callback'    => 'snn_edu_comment_callback',
            'avatar_size' => $avatar,
            'max_depth'   => 4,
        ...
        $can_delete = snn_edu_user_can_delete_comment();
        $editable = $deletable = [];
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    const ajaxurl   = '<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>',
          nonce     = '<?php echo wp_create_nonce( 'snn_edu_comment_edit_nonce' ); ?>',
    ...
    function deleteComment(li, id){
        ...
        postJSON('snn_edu_comment_delete',{ comment_id:id })
...
add_filter( 'map_meta_cap', 'snn_edu_custom_map_meta_cap_for_comment_edit', 10, 4 );
function snn_edu_custom_map_meta_cap_for_comment_edit( $caps, $cap, $user_id, $args ) {
...
/* AJAX handlers */
add_action( 'wp_ajax_snn_edu_comment_edit', 'snn_edu_comment_edit_ajax' );
function snn_edu_comment_edit_ajax() {
    check_ajax_referer( 'snn_edu_comment_edit_nonce', '_ajax_nonce' );
...
add_action( 'wp_ajax_snn_edu_comment_delete', 'snn_edu_comment_delete_ajax' );
function snn_edu_comment_delete_ajax() {
    check_ajax_referer( 'snn_edu_comment_edit_nonce', '_ajax_nonce' );
    $comment_id = isset( $_POST['comment_id'] ) ? (int) $_POST['comment_id'] : 0;
    if ( ! $comment_id ) {
        wp_send_json_error( 'Invalid data', 400 );
